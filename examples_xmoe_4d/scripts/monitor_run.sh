#!/bin/bash
# monitor_run.sh — async, per-rank pipeline-stall monitor for the NON-SLURM (torchrun) X-MoE-4D runs.
#
...
# Run BACKGROUNDED from xmoe_4d.sh.template so it never blocks training:
...
# Env in  : JOB_ID, NODES, TOTAL_GPUS, HOSTFILE  (all exported by xmoe_4d.sh.template).
...
REMOTE='
  ...
  for p in $(pgrep -u "$(id -u)" -f ELM_PP_launch.py); do
...
while true; do
    ...
    if [ -n "${culprit}" ]; then
        printf '\n  ---- full native stack: %s ----\n\n' "${culprit}" >> "${SUMMARY}"
        NATIVE='p=$(pgrep -u "$(id -u)" -f ELM_PP_launch.py | while read pid; do
                  PYSPY=$(command -v py-spy 2>/dev/null); [ -x "$PYSPY" ] || PYSPY="$HOME/.local/bin/py-spy"
