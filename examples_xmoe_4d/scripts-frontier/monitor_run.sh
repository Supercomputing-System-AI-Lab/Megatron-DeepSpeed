#!/bin/bash
# monitor_run.sh — async, per-rank pipeline-stall monitor for the X-MoE-4D SLURM runs.
#
...
REMOTE='
  ...
  for p in $(pgrep -u $USER -f ELM_PP_launch.py); do
...
while true; do
    ...
    if [ -n "${culprit}" ]; then
        printf '\n  ---- full native stack: %s ----\n\n' "${culprit}" >> "${SUMMARY}"
        ${SSH} "${culprit}" 'p=$(pgrep -u $USER -f ELM_PP_launch.py | while read pid; do
