...
# =========================================================
from memory_calculator_v2 import MemoryPredictor 
...
class PipelineOptimizer:
    ...
    def evaluate_baseline(self, ub: int, strategy: Literal["all", "none"]):
        ...
        # Apply Overheads
        max_time += self.cfg.mb_framework_overhead_ms
        fwd_bwd_time_ms = (num_batches + S - 1) * max_time
        optimizer_time_ms = fwd_bwd_time_ms * self.cfg.optimizer_step_ratio
        total_step_time_ms = fwd_bwd_time_ms + optimizer_time_ms
        
        throughput = (effective_gbs * self.cfg.model_config['seqlen']) / (total_step_time_ms / 1000.0)
        return PartitionResult(ub, effective_gbs, num_batches, max_time, total_step_time_ms, throughput, 0.0, plans)
    ...
    def run_ablation(self, mbs: int):
        ...
        if minimax_bottleneck < float('inf') and minimax_plans:
            ...
            res4 = PartitionResult(
                mbs, self.cfg.target_gbs, num_batches,
                max_stage_time, total_time, throughput, bubble, minimax_plans
            )
            results["Minimax"] = res4
        else:
            print("Config 4 (Minimax): infeasible")
        ...
        if results.get("Minimax"):
            res4 = results["Minimax"]
            sorted_plans = sorted(res4.stage_plans, key=lambda x: x.stage_id)
            layers_str = " ".join(str(p.num_layers) for p in sorted_plans)
            ckpts_str = " ".join(str(p.num_checkpoints) for p in sorted_plans)
            print(f"\n# Config 5: Minimax")
            print(f"MINIMAX_MBS={res4.micro_batch_size}")
            print(f"MINIMAX_NUM_BATCHES={res4.num_batches}")
    ...
    def evaluate_throughput_for_mbs(self, ub: int):
        ...
        max_stage_time += self.cfg.mb_framework_overhead_ms
        fwd_bwd_time_ms = (num_batches + S - 1) * max_stage_time
        optimizer_time_ms = fwd_bwd_time_ms * self.cfg.optimizer_step_ratio
        total_step_time_ms = fwd_bwd_time_ms + optimizer_time_ms
        ...
        return PartitionResult(
            ...
            bottleneck_time_ms=max_stage_time, total_step_time_ms=total_step_time_ms, throughput_tokens_per_sec=throughput,
    ...
    def evaluate_fixed_strategy(self, ub: int, strategy: Literal["all", "none"]):
        ...
        return PartitionResult(ub, effective_gbs, num_batches, max_stage_time, total_step_time_ms, throughput, bubble, plans)
...
def get_args():
    ...
    planner_group.add_argument('--memory_limit_gb', type=float, default=62)
...
if __name__ == "__main__":
    ...
    #     print("="*120)
    #     print("="*120)
    #     print(f"|>>> SCENARIO A: AUTOMATIC OPTIMIZATION (Fixing GBS={target_gbs})")
    ...
    #         else:
    #             print("\n" + "="*80)
    #             print(f"| FAILED: MBS={mbs} with strategy '{args.eval_strategy}' is INFEASIBLE (Out of Memory or Incompatible GBS).")
    #             print("=" * 80)
    # =========================================================
